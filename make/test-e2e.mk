...
e2e-setup-istio: | kind-cluster install $(NEEDS_KUBECTL) $(bin_dir)/scratch/istioctl-$(ISTIO_VERSION)
	$(bin_dir)/scratch/istioctl-$(ISTIO_VERSION) install -y -f ./make/config/istio/istio-config-$(ISTIO_VERSION).yaml $(ISTIO_INSTALL_OPIONS)
...
test-e2e-deps-ambient: ISTIO_INSTALL_OPIONS :=
test-e2e-deps-ambient: ISTIO_INSTALL_OPIONS += --set profile=ambient
